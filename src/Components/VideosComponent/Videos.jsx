        ...
        <Stack direction={["column", "row"]} h={"100vh"}>
            <VStack w={"full"}>
                ...
                <VStack alignItems={"flex-start"} p={"8"} w={"full"} overflowY={"auto"}>
                    ...
                    <Text>
                        This is a sample video for testing and demo. It's a description.
                    </Text>
                </VStack>
            </VStack>
            <VStack w={["full", "xl"]} alignItems={"stretch"} pw="8" spacing={"8"} overflowY={"auto"}>
                {
                    videosArr.map((item, index)=>(
                        <Button variant={"ghost"} colorScheme={"blue"} onClick={()=>setVideoSrc(item)}>Lecture {index + 1}</Button>
                    ))
                }         
            </VStack>
        </Stack>
